...
> [!WARNING]
> **PhD Atlas 当前是 Beta（`0.1.0-beta.1`）。** 在第一个稳定公共版之前，
> 数据库结构、已存数据和升级路径可能发生不向后兼容的变化。重要数据必须备份，
> Beta 更新应先在副本中测试。正式的数据和数据库迁移兼容保证从稳定公共版开始。

PhD Atlas 把申请项目、潜在导师、材料、截止日期、通信、奖学金、可复用个人资料、
导出和备份集中到一个安静高效的工作空间中。它面向自托管设计：SQLite 数据库、
上传文件、备份、凭据和集成设置都保留在你控制的服务器上。

本仓库是**公共单工作空间版本**。Team 和机构协作当前没有开放，已列入
[路线图](TODO.zh-CN.md)。今后如何安全启用和发布 Team，详见
[Team 启用指南](TEAM_ENABLEMENT.zh-CN.md)。
...
- 新部署首次打开 `/admin` 时，通过一次性引导创建首位管理员并配置 SMTP；
  连接验证成功后初始化入口永久关闭。
- 在 Admin 上传经过验证的 GitHub Release 更新包；原生服务通过受保护安装器重启，
  自动备份运行时代码并在失败时回滚。
...
- 移除 Team 导航和工作空间切换；
- 拒绝 Team API 路由；
- 移除 Team 邀请处理和 Team 套餐展示；
- 使用空白登录字段，不提供私有演示快捷方式。

Team 协作将在权限、数据迁移和移动端交互模型达到公开使用标准后进入公共版。
...
生产环境由同一个 Node.js 进程提供前端和 API。持久数据保存在 `storage/`。
...
PhD Atlas 会在保存前验证 SMTP 连接。管理员创建成功后，设置 API 永久关闭，
`/admin` 以后只显示正常登录页。公共版不附带默认密码。
...
服务启动后，首次打开 `https://你的域名/admin` 创建管理员并配置系统发件邮箱。
...
SQLite 使用 WAL 时不能只复制主 `.sqlite` 文件。
...
Windows 或 Linux 原生部署更新：

1. 在 Admin 创建完整系统备份。
2. 从目标 GitHub Release 下载 `.tar.gz`，可同时核对 `.sha256`。
3. 打开 **管理后台 → 系统信息 → 系统更新** 并上传。
4. 等待服务重启，重新登录并确认显示的版本。

更新器不会替换 `.env`、SQLite 数据库、上传文件或备份。
Docker 部署应通过 Compose 更新镜像，详见[部署指南](DEPLOYMENT.zh-CN.md)。

## 项目结构

```text
src/                 React 应用、类型化 API 客户端、i18n 和样式
server/              Express 路由、SQLite 仓储、邮件、推送、AI 和导出
