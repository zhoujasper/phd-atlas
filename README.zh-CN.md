...
每个完整发布的 Release 都会在服务端、更新包、数据库与容器门禁通过后，从同一
标签源码生成原生桌面包：

- Windows x64 安装版与便携版 `.exe`；
- 文件名标明原生架构的 macOS `.dmg` 与 `.zip`；
- 每个桌面包旁边一份独立的 `.sha256` 校验文件。

只从[官方 Releases 页面](https://github.com/zhoujasper/phd-atlas/releases)下载，
打开前必须验证对应 SHA-256。当前桌面包尚未签名，因此 Windows SmartScreen 或
macOS Gatekeeper 可能要求用户明确确认。桌面版默认启动为本地、个人、无配额
工作空间，支持本地解锁密码和完整导出，也可以选择连接已有 PhD Atlas 网页账号，
使用远端存储与分享链接。
