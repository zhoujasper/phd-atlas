...
export const phdApi = {
  ...
  sendInitialSetupSmtpVerification: (input: InitialSetupSmtpVerificationInput) =>
    request<{ token: string; expiresInSeconds: number }>('/api/setup/smtp-verification/send', undefined, {
      method: 'POST',
      body: JSON.stringify(input),
    }, 30_000),
  ...
  bootstrapSecrets: (token: string) =>
    request<BootstrapSecrets>('/api/admin/bootstrap-secrets', token),
