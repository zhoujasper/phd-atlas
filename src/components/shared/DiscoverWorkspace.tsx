...
import { TableCell, TableColGroup, TableHeaderCell, useTableColumnMenu } from './TableColumnChrome'
...
export type DiscoverProgramSort = 'program' | 'location' | 'match' | 'funding' | 'deadline' | 'advisors'
...
function useSmoothListReflow<T extends HTMLElement>() {
  ...
  useLayoutEffect(() => {
    ...
    const nextKeys = rows
      ...
      .map((row) => row.dataset.discoverReflowKey)
...
function useAnimatedListPresence<T extends { id: string }>(items: T[], skipExitIds: string[] = []) {
  ...
  useEffect(() => {
    ...
    const skippedIds = new Set(skipExitIds)
    setEntries((current) => {
      const currentById = new Map(current.map((entry) => [entry.item.id, entry]))
      const next: AnimatedListEntry<T>[] = items.map((item) => {
  ...
  return {
    ...
    enteringIds: entries.filter((entry) => entry.phase === 'entering').map((entry) => entry.item.id),
    exitingIds: entries.filter((entry) => entry.phase === 'exiting').map((entry) => entry.item.id),
  }
}
    ...
    <TableHeaderCell column={column} api={api} aria-sort={active ? (direction === 'asc' ? 'ascending' : 'descending') : 'none'}>
      <div className="discover-table-heading">
        <button type="button" className={clsx('discover-table-sort', active && 'is-active')} onClick={() => onSort(sort)}>
          <span>{children}</span>
          {active ? (
            <span className="discover-table-sort-icon" aria-hidden="true">
...
function ProgramList({
  ...
  const columns = useMemo<TableColumnDef[]>(() => [
    { id: 'compare', label: tx('discover.compareMode', 'Compare'), defaultWidth: 38, minWidth: 38, maxWidth: 38, hideable: false, resizable: false },
    ...
    { id: 'actions', label: tx('table.actions', 'Actions'), defaultWidth: 70, minWidth: 70, maxWidth: 70, hideable: false, resizable: false },
  ], [tx])
  ...
  const mobileResultsRef = useSmoothListReflow<HTMLDivElement>()
  const tableBodyRef = useSmoothListReflow<HTMLTableSectionElement>()
        ...
        {filterMotion.items.length ? (
          ...
          <div ref={mobileResultsRef} className="discover-mobile-results">
            {visiblePrograms.map((program) => {
              ...
              return (
                <article key={program.id} data-discover-reflow-key={program.id} className={clsx('discover-mobile-result', selected && 'is-selected', program.hidden && 'is-hidden', hiddenMotion.hidingIds.includes(program.id) && 'is-hiding', filterMotion.enteringIds.includes(program.id) && 'is-filter-entering', filterMotion.exitingIds.includes(program.id) && 'is-filter-exiting', importingId === program.id && 'is-importing')}>
                  <div className="discover-mobile-card-actions" onClick={(event) => event.stopPropagation()}>
                    <button type="button" className={clsx('discover-mobile-card-action', visuallyWatched && 'is-active', watchAnimatingIds.includes(program.id) && 'is-animating')} onClick={() => toggleWatch(program)} aria-label={visuallyWatched ? tx('discover.unwatch') : tx('discover.watch')} aria-pressed={visuallyWatched}>
                      {visuallyWatched ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}
                    </button>
                    <button type="button" className="discover-mobile-card-action" disabled={hiddenMotion.hidingIds.includes(program.id)} onClick={() => hiddenMotion.requestToggle(program.id, Boolean(program.hidden))} aria-label={program.hidden ? tx('discover.restore') : tx('discover.hide')}>
                      {program.hidden ? <Eye size={16} /> : <EyeOff size={16} />}
                    </button>
                  </div>
                  <button type="button" className="discover-mobile-result-main" onClick={() => actions.selectProgram(program.id)}>
          ...
          <table className="discover-data-table atlas-table" style={{ width: tableWidth, minWidth: '100%' }} onContextMenu={(event) => openMenu(event, tx('table.columns', 'Columns'))}>
            ...
            <thead className="discover-program-head">
              <tr>
                <TableHeaderCell column={column('compare')} api={api}><span aria-hidden="true" /></TableHeaderCell>
            ...
            <tbody ref={tableBodyRef}>
              {visiblePrograms.map((program) => {
                ...
                const deadline = programDeadline(program)
                return (
                  <tr key={program.id} data-discover-reflow-key={program.id} className={clsx('discover-program-row', selected && 'is-selected', program.hidden && 'is-hidden', hiddenMotion.hidingIds.includes(program.id) && 'is-hiding', filterMotion.enteringIds.includes(program.id) && 'is-filter-entering', filterMotion.exitingIds.includes(program.id) && 'is-filter-exiting', importingId === program.id && 'is-importing')} onClick={() => actions.selectProgram(program.id)}>
                    <TableCell columnId="compare" api={api} className="discover-compare-cell">
                      <label className="discover-compare-check" onClick={(event) => event.stopPropagation()}>
                        <input className="discover-checkbox-input" type="checkbox" checked={compared} onChange={() => actions.toggleCompare(program.id)} aria-label={tx('discover.addToCompare', 'Add to compare')} />
                        <span className="discover-checkbox-visual" aria-hidden="true"><Check size={11} /></span>
                      </label>
                    </TableCell>
                    <TableCell columnId="program" api={api}><button type="button" className="discover-program-identity" onClick={() => actions.selectProgram(program.id)}><strong>{program.school}</strong><span>{program.program}</span></button></TableCell>
                    ...
                    <TableCell columnId="actions" api={api} className="discover-actions-cell">
                      <div className="discover-row-actions" onClick={(event) => event.stopPropagation()}>
                        <button type="button" className={clsx('discover-icon-btn', visuallyWatched && 'active', watchAnimatingIds.includes(program.id) && 'is-watch-animating')} onClick={() => toggleWatch(program)} aria-label={visuallyWatched ? tx('discover.unwatch') : tx('discover.watch')} aria-pressed={visuallyWatched}>{visuallyWatched ? <BookmarkCheck size={16} /> : <Bookmark size={16} />}</button>
                        <button type="button" className="discover-icon-btn hover-reveal" disabled={hiddenMotion.hidingIds.includes(program.id)} onClick={() => hiddenMotion.requestToggle(program.id, Boolean(program.hidden))} aria-label={program.hidden ? tx('discover.restore') : tx('discover.hide')}>{program.hidden ? <Eye size={16} /> : <EyeOff size={16} />}</button>
...
function InspectorSection({ title, summary, children, open = false }: { title: string; summary?: string; children: ReactNode; open?: boolean }) {
  return (
    <SmoothDisclosure
  ...
  return (
    <aside className={clsx('discover-inspector', mobileOpen && 'is-mobile-open')} aria-label={tx('discover.programDetails', 'Program details')} aria-hidden={collapsed && !mobileOpen} inert={collapsed && !mobileOpen}>
      ...
      <div className="discover-inspector-scroll">
        ...
        <InspectorSection title={tx('discover.applicationRules', 'Application rules')} summary={multiApplyLabel(program.multiApply, lang)}>
          <dl className="discover-detail-list">
        ...
        <InspectorSection title={tx('discover.internationalStudents', 'International students')} summary={intlEligible === false ? tx('discover.restrictionsApply', 'Restrictions apply') : tx('discover.eligible', 'Eligible')}>
        ...
        <InspectorSection title={tx('discover.personalNote', 'Personal note')} summary={note.trim() ? tx('discover.noteSaved', 'Saved') : tx('discover.addNote', 'Add note')}>
        ...
        <InspectorSection title={tx('discover.officialSources', 'Official sources')} summary={tx('discover.sourceCount', '{count} sources').replace('{count}', String(program.sources.length))}>
  ...
  return (
    <aside className={clsx('discover-inspector', mobileOpen && 'is-mobile-open')} aria-label={tx('discover.advisorDetails', 'Advisor details')} aria-hidden={collapsed && !mobileOpen} inert={collapsed && !mobileOpen}>
      ...
      <div className="discover-inspector-scroll">
        ...
        <InspectorSection title={tx('discover.labProfile', 'Lab profile')} summary={pi.labSize || tx('discover.unknown', 'Unknown')}>
          <dl className="discover-detail-list">
        ...
        <InspectorSection title={tx('discover.originalResearchNote', 'Original research note')} summary={tx('discover.expandToRead', 'Expand to read')}>
        ...
        <InspectorSection title={tx('discover.links', 'Links')} summary={tx('discover.officialSources', 'Official sources')}>
          <div className="discover-source-list">
...
export function DiscoverWorkspace(props: DiscoverWorkspaceProps) {
  ...
  const {
    ...
    piNoteDrafts, importingId, researching, saving, hiddenProgramCount, hiddenPiCount, actions,
  } = props
  ...
  const [dragPreview, setDragPreview] = useState<{ pane: 'filters' | 'inspector'; width: number } | null>(null)
  ...
  useEffect(() => {
    ...
    const keepCompactModeAvailable = () => {
      if (compactQuery.matches && mode === 'compare') actions.setMode('programs')
    }
  ...
  const startPaneResize = useCallback((pane: 'filters' | 'inspector', event: ReactPointerEvent<HTMLButtonElement>) => {
    ...
    if (!startCollapsed) setDragPreview({ pane, width: initialVisibleWidth })
    ...
    const onMove = (moveEvent: globalThis.PointerEvent) => {
      ...
      lastWidth = nextWidth
      setDragPreview({ pane, width: nextWidth })
    }
    const stop = () => {
      document.body.classList.remove('discover-pane-resizing')
  ...
  const workspaceStyle = {
    '--discover-filter-width': `${dragPreview?.pane === 'filters' ? dragPreview.width : layout.filterWidth}px`,
    '--discover-inspector-width': `${dragPreview?.pane === 'inspector' ? dragPreview.width : layout.inspectorWidth}px`,
  } as CSSProperties
  const modeTabsStyle = {
    '--discover-tab-index': ['programs', 'pis', 'compare'].indexOf(mode),
  } as CSSProperties
  return (
    <div className={clsx('discover-v2', `mode-${mode}`, `mode-${modeDirection}`, mode === 'compare' && 'is-compare', filterRailCollapsed && 'filters-collapsed', !inspectorOpen && 'inspector-closed', resizingPane && 'is-resizing')}>
      ...
      <main ref={workspaceRef} className="discover-workspace" style={workspaceStyle}>
        ...
        {mode === 'programs' ? <ProgramList programs={programs} selectedProgram={selectedProgram} compareIds={compareIds} scoreByProgramId={scoreByProgramId} currencyCode={meta?.currency || 'USD'} importingId={importingId} programSort={programSort} sortDirection={sortDirection} actions={actions} /> : null}
      ...
      <div className="discover-statusbar">
